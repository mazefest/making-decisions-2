...
nums.forEach(findEvens);

function findEvens(item, index, arr) {
  if (item % 2 === 0) {
    evensArr.push(item);
  }
}
...
let retrievingIndex = 7;
if (retrievingIndex <= myFavoriteNumbers.length - 1); // -1 for last index
...
for (let i = 1; i <= 100; i ++) {
  let isDev = i % 3 === 0; 
  let isMountain = i % 5 === 0 ;
  let isDevMountain = isDev && isMountain;
  let isNot = !isDev && !isMountain;
